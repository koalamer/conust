...
// For integers you can use the built in strconv.FormatInt
...
// WithBaseDecimals sets how many fractional digits in the given base should be kept when converting to string
...
// WithDecimals sets a fractional precision high enough to ensure the given base(10) precision
...
// Choose a value that is large enough to cover (most of) your result strings but does not allocate needlessly large amounts of memory
func (fc *FloatConverter) WithTypicalLength(length int) {
	fc.typicalLength = length
}

// NewFloatConverter returns an initialized base converter
...
// FormatFloat converts the number to a string in the specified base and with the specified fractional precision
